...
class PlayerBackendLibUADE(PlayerBackend):
    ...
    def load_module(self, module_filename: str) -> bool:
        ...
        ret = libuade.uade_play_from_buffer(None, ret, self.module_size, -1, self.state)
        # info = libuade.uade_get_song_info(self.state)
    ...
    def read_chunk(self, samplerate: int, buffersize: int) -> tuple[int, bytes]:
        ...
        nbytes = libuade.uade_read(buf, ctypes.sizeof(buf), self.state)
    ...
    def get_event(self) -> uade_event:
        ...
        e = libuade.uade_get_event(ctypes.byref(event), self.state)
